...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('teams_notification_settings', function (Blueprint $table) {
            ...
            $table->boolean('server_patch_teams_notifications')->default(false);
        ...
        foreach ($teams as $team) {
            try {
                DB::table('teams_notification_settings')->insert([
                    'team_id' => $team->id,
                ]);
            } catch (\Throwable $e) {
                Log::error('Error creating teams notification settings for existing teams: '.$e->getMessage());
            }
        }
    }
